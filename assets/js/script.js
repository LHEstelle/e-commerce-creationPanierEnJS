  ...
  .then((data) => {
    data.results.forEach((element, index) => {
      ...
      <div class="card-body">
          ...
          <p id="categorie${index}" class="categorie"></p>
          <p id="description${index}" class="description">${element.description}</p>
          <div class= "d-flex justify-content-between">
          <a  id="price${index}" class="btn btn-outline-light">${element.price}</a>
          <button class="btn btn-outline-primary" data-acheter data-price="${element.price}" data-reference="${element.reference}" data-article="${element.title}">Acheter</button>
          </div>
          </div>
          </div>
            `;
...
function showArticle(article) {
  if (article == "teeShirt") {
    ...
    document.querySelectorAll('[data-cat="pantalon"]').forEach((element) => {
...
document.getElementById("allCards").addEventListener("click", (event) => {
  if (event.target.hasAttribute("data-acheter")) {
    if (arrayCart.includes(event.target.dataset.reference)) {
      document.getElementById(`quantite-${event.target.dataset.reference}`)
        .textContent++;
      ...
      document.getElementById("myTable").innerHTML += `
          <tr id="line-${event.target.dataset.reference}">
            ...
            <td><button class="btn btn-secondary btn-sm" onclick="changeQuantity(${event.target.dataset.reference}, 'minus')">-</button></td>
            <td class="text-center" id="quantite-${event.target.dataset.reference}">1</td>
            <td><button class="btn btn-secondary btn-sm" onclick="changeQuantity(${event.target.dataset.reference}, 'plus')">+</button></td>
            <td id="total-${event.target.dataset.reference}" data-total></td>
...
function changeQuantity(ref, action) {
  if (
    action == "plus" &&
    document.getElementById(`quantite-${ref}`).textContent < 30
  ) {
  ...
  if (
    action == "minus" &&
    document.getElementById(`quantite-${ref}`).textContent > 1
  ) {
    document.getElementById(`quantite-${ref}`).textContent--;
    calculateSousTotal(ref);
    calculateTotal();
...
function calculateSousTotal(ref) {
  let sousTotal =
    parseInt(document.getElementById(`price-${ref}`).textContent) *
    Number(document.getElementById(`quantite-${ref}`).textContent);
  document.getElementById(`total-${ref}`).textContent = sousTotal;
}

function calculateTotal() {
  let total = 0;
  let totalArray = document.querySelectorAll("[data-total]");
